...
%         m2_yi_value = diag(m2_yi_function(t_value));
        ...
        %% With Oscialltion
        A11 = -1i.*kx.*diag(U_yi_function(t_value)).*Lap + 1i.*kx.*diag(m2_yi_function(t_value)) + (1/Re).*Lap2 + 1i.*kz.*(diag(W_m2_yi_function(t_value) - diag(W_yi_function(t_value).*Lap)));
        A12 = zeros(Ny);
        A21 = -1i.*kz.*diag(m_yi_function(t_value)) + 1i.*kx.*diag(W_m_yi_function(t_value));
        A22 = -1i.*kx.*diag(U_yi_function(t_value)) + (1/Re).*Lap - 1i.*kz.*diag(W_yi_function(t_value));
